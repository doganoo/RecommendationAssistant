...
class ChangedFilesManager{
	...
	public function deleteOutdated(){
		//TODO implement
		ConsoleLogger::warn("method is not implemented");
	}
	...
	public function allFiles(): ArrayList{
		...
		$query->select(DbConstants::TB_CFL_FILE_ID, DbConstants::TB_CFL_USER_ID)
		      ->from(DbConstants::TABLE_NAME_CHANGED_FILES_LOG);
		$result = $query->execute();
		while(false !== $row = $result->fetch()){
			$file = NodeUtil::getFile($row[DbConstants::TB_CFL_FILE_ID], $row[DbConstants::TB_CFL_USER_ID]);
			if(null === $file) continue;
			$item = new Item();
			$item->setId($file->getId());
			$item->setOwnerId($file->getOwner()->getUID());
			$item->setName($file->getName());
			//TODO set item rating
		}
		ConsoleLogger::warn("items have no ratings");
		$result->closeCursor();
		return $list;
	}
